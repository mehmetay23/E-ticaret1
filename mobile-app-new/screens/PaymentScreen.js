...
const PaymentScreen = () => {
  ...
  const { address, orderSummary } = route.params || {};
  ...
  // Sipariş özeti hesaplamaları - route params'tan gelen değerleri kullan
  const subtotal = orderSummary?.subtotal || cart.reduce((sum, item) => sum + (parseFloat(item.price) * (item.quantity || 1)), 0);
  const shipping = orderSummary?.shipping || (subtotal > 150 ? 0 : 29.90);
  const tax = orderSummary?.tax || (subtotal * 0.18);
  const total = orderSummary?.total || (subtotal + shipping + tax);
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (parseInt(month) < 1 || parseInt(month) > 12) {
        Alert.alert('Hata', 'Geçersiz son kullanma ayı.');
        return;
      }
      
      if (parseInt(year) < currentYear || (parseInt(year) === currentYear && parseInt(month) < currentMonth)) {
        Alert.alert('Hata', 'Kartınızın son kullanma tarihi geçmiş.');
        return;
      }
      ...
      if (response.ok) {
        ...
        Alert.alert('Hata', data.message || 'Ödeme işlemi başarısız oldu. Lütfen tekrar deneyin.');
      }
    } catch (error) {
      console.error('Ödeme hatası:', error);
      Alert.alert('Hata', 'Ödeme işlemi sırasında bir hata oluştu. Lütfen daha sonra tekrar deneyin.');
    } finally {
      setLoading(false);
    }
  };
